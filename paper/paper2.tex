...
\begin{abstract}
Multi-agent decision systems increasingly use LLMs as orchestration layers, yet the conditions under which inference overhead is justified remain poorly understood. This paper identifies a precise environmental condition: \textbf{LLM orchestration is beneficial in high-friction environments where action selectivity dominates signal frequency} --- where skipping a low-quality action is worth more than acting frequently. We demonstrate this using AgenticTraders, an LLM-orchestrated trading system, over a 45-day intraday backtest on 147 US equities with realistic transaction costs (10 bps slippage + \$1 commission per leg). Under zero transaction costs, rule-based baselines (UCB-bandit, equal-weight ensemble) match or exceed the LLM Orchestrator --- the overhead is not justified. Adding realistic costs inverts the ranking: the Orchestrator+LLM ranks first among all architectures, because its selective entry suppresses low-quality trades whose cumulative friction outweighs any signal advantage. To characterize where the overhead goes, we decompose per-cycle latency across four controlled experiments. The LLM call accounts for 99.8\% of total cycle time (8,617 ms of 8,633 ms); feature computation (156 ms) and agent calls (15 ms) are negligible. Feature computation is the only stage that scales with universe size ($O(n)$); LLM latency grows sub-linearly due to prompt summarization; agent count adds only $\sim$420 ms per additional agent. Strategy agents are intentionally kept simple (heuristic scorers) to isolate overhead attribution from strategy complexity --- a design choice that lets the latency decomposition remain clean regardless of downstream alpha quality. The practical upshot: LLM orchestration overhead is a systems-design question, not a model-quality question. Deploy it where per-trade friction is high and cadence is low; prefer lightweight bandits or ensembles where friction is negligible or latency is critical.
\end{abstract}
...
Five extensions address the paper's acknowledged weaknesses directly.

\textbf{1. Longer-horizon, multi-regime evaluation.}
The 45-day intraday window covers a single market regime, making return comparisons statistically fragile and annualized extrapolations approximate. Running the same experiments on 1-hour bars over a 1--2 year window would (a) increase decision cycles from $\sim$78 to $\sim$1,000+, providing power for latency distribution analysis; (b) test whether the action-selectivity advantage holds across bull, bear, and choppy regimes; and (c) produce annualized returns that do not require extrapolation. The hourly cadence also places LLM latency ($\sim$6 s) well within the decision window, removing real-time feasibility concerns. This single extension would address the most common reviewer objection to the current results.

\textbf{2. Stronger and learned strategy baselines.}
The heuristic scorers (TrendAgent, MomentumAgent, MeanReversionAgent) are intentionally simple to isolate overhead attribution, but a reviewer may ask whether the LLM's selectivity advantage persists against stronger baselines. Replacing or augmenting the heuristic agents with (a) a deep Q-network (DQN) or PPO-based RL agent trained on historical data, (b) a Thompson-sampling bandit with learned reward distributions, or (c) a cross-sectional momentum factor model would test whether the action-selectivity finding is robust to strategy quality. If the LLM Orchestrator still ranks first under slippage against an RL baseline, the result is significantly stronger.

\textbf{3. System-level latency reduction: caching, batching, and model downsizing.}
The 99.8\% LLM dominance result motivates asking: how much of that overhead can be recovered? Three interventions should be benchmarked: (a) \textit{prompt compression} --- truncating agent ideas from 10 to 3 per agent, reducing tokens by $\sim$40\%; (b) \textit{response caching} --- reusing the previous cycle's decision when the market snapshot has not changed beyond a volatility threshold; (c) \textit{model downsizing} --- replacing GPT-4o-mini with a 7B local model (e.g., Mistral-7B via Ollama) to reduce per-call latency from $\sim$6 s to $\sim$0.3 s; and (d) \textit{async batching} --- issuing the LLM call for bar $t$ while the simulation processes bar $t-1$, decoupling inference from the critical path. Each produces a (latency, return) pair enabling a Pareto frontier: the minimum viable LLM budget for a target selectivity level. This extension would transform the paper from a measurement study into a systems optimization paper.

\textbf{4. Multi-provider latency comparison.}
All results use GPT-4o-mini via the OpenAI API. Whether the 99.8\% dominance result is a property of the architecture or an artifact of this provider's infrastructure is unknown. A controlled comparison across GPT-4o-mini, Anthropic Claude Haiku, Google Gemini Flash, and a locally-hosted 7B model --- using identical prompts and decision logic --- would establish provider-agnostic latency bounds and identify whether a lower-latency provider changes the optimal decision cadence.

\textbf{5. Overhead measurement on a verified profitable strategy.}
The current evaluation uses a losing strategy ($-0.24\%$ over 45 days), which is appropriate for isolating overhead attribution but leaves open the question of whether the findings transfer to profitable systems. Repeating the latency decomposition on a strategy with verified positive expectancy --- e.g., a momentum factor on a longer horizon, or a pairs-trading strategy with documented edge --- would (a) confirm that action selectivity still dominates under realistic costs when the underlying signal has positive value, and (b) allow the break-even analysis to produce economically meaningful cost thresholds rather than comparisons among negative-return agents.
